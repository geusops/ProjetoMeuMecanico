...
useEffect(() => {
  if (oficinaSelecionada) {
    axios.get(`http://localhost:3000/avaliacoes/${oficinaSelecionada.id_oficina}`)
    ...
    <div>
      ...
      {mostrarForm && (
        <div className="border rounded-md p-6 mt-4 bg-slate-50">
          <h2 className="text-xl font-bold text-gray-800 mb-4">Deixe sua avaliação</h2>
          <form onSubmit={handleAvaliar} className="space-y-4">
            {/* Nota */}
            <div>
              <label className="text-gray-700 font-medium">Nota (1 a 5)</label>
              <select
                value={nota}
            ...
            {mensagemAvaliacao && <p className="font-semibold mt-2">{mensagemAvaliacao}</p>}
          </form>
        </div>
      )}
